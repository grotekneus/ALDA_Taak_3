...
//! [0]
Node::Node(GraphWidget *graphWidget,QChar name,bool infected)
    : infected(infected),name(name),graph(graphWidget)
{
...
void Node::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *)
{
    ...
    if (option->state & QStyle::State_Sunken) {
        ...
        if(infected){
            ...
            gradient.setColorAt(1, QColor(Qt::cyan));
            gradient.setColorAt(0, QColor(Qt::darkCyan));
        }
    }
    ...
    painter->setFont(QFont("Arial", 8));  // Adjust font size if needed
    painter->drawText(-3, 3, name);
}
//! [10]
